...
Multiplot <- function(chain1, chain2, chain3){
  ...
  # plot(chain1$Accept_Fraction, type = "l", main = "Acceptance Fraction", xlab = "index", col = "black")
  # lines(chain2$Accept_Fraction, col = "red")
  # lines(chain3$Accept_Fraction, col = "blue")
  
  par(oldPar)
}

Extraplot <- function(chain1, chain2, chain3){
  
  oldPar <- par(mfrow = c(2, 3), pty = "s")

  h1 <- density(chain1$MC_param.7)
  h2 <- density(chain2$MC_param.7)
  h3 <- density(chain3$MC_param.7)
  ymax <- max(c(h1$y, h2$y, h3$y)) * 1.05
  plot(h1$x, h1$y, type = "l", col = "black", xlim = range(chain1$MC_param.7), 
       ylim = c(0, ymax), main = "Posterior Parameter 7", 
       xlab = "x", ylab = "Density")
  lines(h2$x, h2$y, col = "red")
  lines(h3$x, h3$y, col = "blue")
  
  h1 <- density(chain1$MC_param.8)
  h2 <- density(chain2$MC_param.8)
  h3 <- density(chain3$MC_param.8)
  ymax <- max(c(h1$y, h2$y, h3$y)) * 1.05
  plot(h1$x, h1$y, type = "l", col = "black", xlim = range(chain1$MC_param.8), 
       ylim = c(0, ymax), main = "Posterior Parameter 8", 
       xlab = "x", ylab = "Density")
  lines(h2$x, h2$y, col = "red")
  lines(h3$x, h3$y, col = "blue")
  
  h1 <- density(chain1$MC_param.9)
  h2 <- density(chain2$MC_param.9)
  h3 <- density(chain3$MC_param.9)
  ymax <- max(c(h1$y, h2$y, h3$y)) * 1.05
  plot(h1$x, h1$y, type = "l", col = "black", xlim = range(chain1$MC_param.9), 
       ylim = c(0, ymax), main = "Posterior Parameter 9", 
       xlab = "x", ylab = "Density")
  lines(h2$x, h2$y, col = "red")
  lines(h3$x, h3$y, col = "blue")
  
  plot(chain1$MC_param.7, type = "l", main = "", ylab = "Target 7 Sample")
  lines(chain2$MC_param.7, col = "red")
  lines(chain3$MC_param.7, col = "blue")
  
  plot(chain1$MC_param.8, type = "l", main = "", ylab = "Target 8 Sample")
  lines(chain2$MC_param.8, col = "red")
  lines(chain3$MC_param.8, col = "blue")
  
  plot(chain1$MC_param.9, type = "l", main = "", ylab = "Target 9 Sample")
  lines(chain2$MC_param.9, col = "red")
  lines(chain3$MC_param.9, col = "blue")
...
#Likelihood evaluation for NSG mice injected with B16F0 WT cells as control group for both DNMT3A and PTPM11 KO groups
LLH_DPWT_NSG <- function(theta, data){
  Yhat1 <- theta[1] * data[1:7,1] + theta[2] 
  SSE1 <- sum((data[1:7,2] - Yhat1)^2) 
  ...
  Yhat3 <- theta[1] * data[15:20,1] + theta[4] 
  SSE3 <- sum((data[15:20,2] - Yhat3)^2) 
  
  Yhat4 <- theta[1] * data[21:27,1] + theta[5] 
  SSE4 <- sum((data[21:27,2] - Yhat4)^2)
  
  Yhat5 <- theta[1] * data[28:34,1] + theta[6] 
  SSE5 <- sum((data[28:34,2] - Yhat5)^2) 
...
T_DPWT_NSG1 <- MHmcmc(Sig, LLH_DPWT_NSG, DPWT_NSG, steps = 100000, target = 0.2, randomSeed = 1, startValue = c(.3, 3, 3, 3, 3, 3))
T_DPWT_NSG2 <- MHmcmc(Sig, LLH_DPWT_NSG, DPWT_NSG, steps = 100000, target = 0.2, randomSeed = 2, startValue = c(.2, 2.75, 2.5, 2.75, 2.25, 2.5))
T_DPWT_NSG3 <- MHmcmc(Sig, LLH_DPWT_NSG, DPWT_NSG, steps = 100000, target = 0.2, randomSeed = 3, startValue = c(.5, 1, 1, 1, 1, 1))
...
#Likelihood evaluation for NSG mice injected with DNMT3A KO cells
LLH_D_NSG <- function(theta, data){
  Yhat1 <- theta[1] * data[1:7,1] + theta[2] 
  SSE1 <- sum((data[1:7,2] - Yhat1)^2) 
  ...
  Yhat5 <- theta[1] * data[29:34,1] + theta[6] 
  SSE5 <- sum((data[29:34,2] - Yhat5)^2) 
...
T_D_NSG1 <- MHmcmc(Sig, LLH_D_NSG, D_NSG, steps = 100000, target = 0.2, randomSeed = 7, startValue = c(.3, 3, 3, 3, 3, 3))
T_D_NSG2 <- MHmcmc(Sig, LLH_D_NSG, D_NSG, steps = 100000, target = 0.2, randomSeed = 8, startValue = c(.2, 2.75, 2.5, 2.75, 2.25, 2.5))
T_D_NSG3 <- MHmcmc(Sig, LLH_D_NSG, D_NSG, steps = 100000, target = 0.2, randomSeed = 9, startValue = c(.5, 1, 1, 1, 1, 1))
...
LLH_P_C57 <- function(theta, data){
  ...
  Yhat3 <- theta[1] * data[10:12,1] + theta[4] 
  SSE3 <- sum((data[10:12,2] - Yhat3)^2) 
  
  Yhat4 <- theta[1] * data[13:18,1] + theta[5] 
  SSE4 <- sum((data[13:18,2] - Yhat4)^2)
  
  Yhat5 <- theta[1] * data[19:23,1] + theta[6] 
  SSE5 <- sum((data[19:23,2] - Yhat5)^2) 
...
#Likelihood evaluation for WISP1 WT Control in NSG mice
LLH_WWT_NSG <- function(theta, data){
  Yhat1 <- theta[1] * data[1:5,1] + theta[2] 
  SSE1 <- sum((data[1:5,2] - Yhat1)^2) 
  
  Yhat2 <- theta[1] * data[6:12,1] + theta[3] 
  SSE2 <- sum((data[6:12,2] - Yhat2)^2) 
  
  Yhat3 <- theta[1] * data[13:18,1] + theta[4] 
  SSE3 <- sum((data[13:18,2] - Yhat3)^2) 
  
  Yhat4 <- theta[1] * data[19:25,1] + theta[5] 
  SSE4 <- sum((data[19:25,2] - Yhat4)^2)
  
  Yhat5 <- theta[1] * data[26:31,1] + theta[6] 
  SSE5 <- sum((data[26:31,2] - Yhat5)^2) 
...
T_WWT_NSG1 <- MHmcmc(Sig, LLH_WWT_NSG, WWT_NSG, steps = 100000, target = 0.2, randomSeed = 37, startValue = c(.3, 3, 3, 3, 3, 3))
T_WWT_NSG2 <- MHmcmc(Sig, LLH_WWT_NSG, WWT_NSG, steps = 100000, target = 0.2, randomSeed = 38, startValue = c(.2, 2.75, 2.5, 2.75, 2.25, 2.5))
T_WWT_NSG3 <- MHmcmc(Sig, LLH_WWT_NSG, WWT_NSG, steps = 100000, target = 0.2, randomSeed = 39, startValue = c(.5, 1, 1, 1, 1, 1))
...
LLH_WWT_C57 <- function(theta, data){
  ...
  Yhat6 <- theta[1] * data[27:31,1] + theta[7] 
  SSE6 <- sum((data[27:31,2] - Yhat6)^2) 
  
  Yhat7 <- theta[1] * data[32:36,1] + theta[8] 
  SSE7 <- sum((data[32:36,2] - Yhat7)^2) 
  
  Yhat8 <- theta[1] * data[37:39,1] + theta[9] 
  SSE8 <- sum((data[37:39,2] - Yhat8)^2) 
  
  post <- (SSE1+SSE2+SSE3+SSE4+SSE5+SSE6+SSE7+SSE8)^(-nrow(data)/2) 
  
  return(post)
}

Sig <- c(.1, 1, 1, 1, 1, 1, 1, 1, 1)

T_WWT_C571 <- MHmcmc(Sig, LLH_WWT_C57, WWT_C57, steps = 100000, target = 0.2, randomSeed = 40, startValue = c(.3, 3, 3, 3, 3, 3, 3, 3, 3))
T_WWT_C572 <- MHmcmc(Sig, LLH_WWT_C57, WWT_C57, steps = 100000, target = 0.2, randomSeed = 41, startValue = c(.2, 2.75, 2.5, 2.75, 2.25, 2.5, 2, 2, 2))
T_WWT_C573 <- MHmcmc(Sig, LLH_WWT_C57, WWT_C57, steps = 100000, target = 0.2, randomSeed = 42, startValue = c(0, 4, 4, 4, 4, 4, 4, 4, 4))

#Convergence
WWT_C571_MCMC <- mcmc(data = T_WWT_C571[,1:9])
WWT_C572_MCMC <- mcmc(data = T_WWT_C572[,1:9])
WWT_C573_MCMC <- mcmc(data = T_WWT_C573[,1:9])
...
# Extraplot(T_WWT_C571, T_WWT_C572, T_WWT_C573)
# dev.off() # End plot

#_______________________________________________________________________________
#Analysis of NSG and C57B/6 mice injected with B16F0 WISP1 KO cells

#Likelihood evaluation for WISP1 KO in NSG mice
LLH_W_NSG <- function(theta, data){
  Yhat1 <- theta[1] * data[1:5,1] + theta[2] 
  SSE1 <- sum((data[1:5,2] - Yhat1)^2) 
  
  Yhat2 <- theta[1] * data[6:11,1] + theta[3] 
  SSE2 <- sum((data[6:11,2] - Yhat2)^2) 
  
  Yhat3 <- theta[1] * data[12:17,1] + theta[4] 
  SSE3 <- sum((data[12:17,2] - Yhat3)^2) 
  
  Yhat4 <- theta[1] * data[18:23,1] + theta[5] 
  SSE4 <- sum((data[18:23,2] - Yhat4)^2)
  
  Yhat5 <- theta[1] * data[24:29,1] + theta[6] 
  SSE5 <- sum((data[24:29,2] - Yhat5)^2) 
...
T_W_NSG1 <- MHmcmc(Sig, LLH_W_NSG, W_NSG, steps = 100000, target = 0.2, randomSeed = 43, startValue = c(.3, 3, 3, 3, 3, 3))
T_W_NSG2 <- MHmcmc(Sig, LLH_W_NSG, W_NSG, steps = 100000, target = 0.2, randomSeed = 44, startValue = c(.2, 2.75, 2.5, 2.75, 2.25, 2.5))
T_W_NSG3 <- MHmcmc(Sig, LLH_W_NSG, W_NSG, steps = 100000, target = 0.2, randomSeed = 45, startValue = c(.5, 1, 1, 1, 1, 1))
...
#Likelihood evaluation for WISP1 KO in C57 Mice
LLH_W_C57 <- function(theta, data){
  ...
  Yhat6 <- theta[1] * data[44:51,1] + theta[7] 
  SSE6 <- sum((data[44:51,2] - Yhat6)^2) 
  
  Yhat7 <- theta[1] * data[52:57,1] + theta[8] 
  SSE7 <- sum((data[52:57,2] - Yhat7)^2) 
  
  Yhat8 <- theta[1] * data[58:66,1] + theta[9] 
  SSE8 <- sum((data[58:66,2] - Yhat8)^2) 
...
T_W_C571 <- MHmcmc(Sig, LLH_W_C57, W_C57, steps = 100000, target = 0.2, randomSeed = 46, startValue = c(.3, 3, 3, 3, 3, 3, 3, 3, 3))
T_W_C572 <- MHmcmc(Sig, LLH_W_C57, W_C57, steps = 100000, target = 0.2, randomSeed = 47, startValue = c(.2, 2.75, 2.5, 2.75, 2.25, 2.5, 2, 2, 2))
T_W_C573 <- MHmcmc(Sig, LLH_W_C57, W_C57, steps = 100000, target = 0.2, randomSeed = 48, startValue = c(.5, 1, 1, 1, 1, 1, 1, 1, 1))

#Convergence
W_C571_MCMC <- mcmc(data = T_W_C571[,1:9])
W_C572_MCMC <- mcmc(data = T_W_C572[,1:9])
W_C573_MCMC <- mcmc(data = T_W_C573[,1:9])
...
# Extraplot(T_W_C571, T_W_C572, T_W_C573)
# dev.off() # End plot